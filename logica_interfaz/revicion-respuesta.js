...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.verRespuestas = async (surveyId) => {
        ...
        try {
            const token = getAuthToken();
            const response = await fetch(`http://localhost:3001/surveys/${surveyId}/respuestas-detalladas`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
                ...
                response.respuestas_pregunta.forEach(pregunta => {
                    detailsHTML += `
                        <div style="margin-bottom: 15px; padding-bottom: 15px; border-bottom: 1px solid #ddd;">
                            <strong>${pregunta.texto_pregunta}</strong><br>
                            <span style="color: #666;">Tipo: ${pregunta.tipo_pregunta}</span><br>
    ...
    window.agregarComentario = async (index) => {
        ...
        try {
            const token = getAuthToken();
            const apiResponse = await fetch(`http://localhost:3001/respuestas/${response.id_respuesta}/comentario`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
